...
This challenge just is a c file. so it easily to analyses.
...
`is_ok` must be 1, so `highly_optimized_parallel_comparsion` function return 1.
...
from this, we can infer the result must be 0. because generated_string[i] == just_a_string[i].

so just modify something a little bit
...
void * checking(void *arg) {
    ...
    *result = (argument[0]+argument[1]) ^ argument[2];
    ...
    printf("%c", (argument[0]+argument[1]) ^ 0);
    return result;
}
```
...
```
// gcc parallel-comparator-200.c -o elf -lpthread
#include <stdlib.h>
#include <stdio.h>
#include <pthread.h>
