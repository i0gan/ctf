...
保护全开, 有添加, 删除, 修改,打印,退出功能, 添加采用calloc方式开辟内存.

## vul

```c
__int64 __fastcall sub_E26(signed int or, unsigned int in)
{
  __int64 result; // rax

  if ( or > (signed int)in )
    return in;
  if ( in - or == 10 ) //当输入的大小比原来的大小打10的时候, 存在off by one漏洞
    LODWORD(result) = or + 1;
  else
    LODWORD(result) = or;
...
off by one, one gadget

## 思路

通过house of einherjar结合fastbin attack 打入malloc_hook -0x23处, 使用realloc调整参数.然后get shell
